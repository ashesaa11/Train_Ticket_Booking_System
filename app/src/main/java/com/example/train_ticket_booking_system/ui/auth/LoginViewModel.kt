    ...
    private fun login(phone: String, password: String) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (user != null) {
                    Log.d("TTBS_AUTH", "login success, phone=$phone")
                    _state.value = _state.value.copy(
                        loading = false, phone = phone
                    )
    ...
    private fun register(phone: String, password: String) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (user != null) {
                    Log.d("TTBS_AUTH", "register success, phone=$phone")
                    _state.value = _state.value.copy(loading = false, phone = phone)
                    _loginEvent.tryEmit(Unit)
                } else {
    ...
    fun devLogin() {
        ...
        viewModelScope.launch {
            try {
                ...
                _state.value = _state.value.copy(loading = false, phone = devPhone)
                _loginEvent.tryEmit(Unit)
            } catch (e: Exception) {
